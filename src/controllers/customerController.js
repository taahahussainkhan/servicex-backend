...
export const getDashboardData = asyncHandler(async (req, res) => {
  const customer = await Customer.findById(req.user.id)
    ...
    .populate(
      "favoriteServians.servian",
      "name averageRating serviceCategory profileImage role"
    );
  ...
  const recentBookings = await Booking.find({ customer: req.user.id })
    .populate({
      path: "servian",
      select: "name profileImage averageRating role serviceCategory phone",
      model: "servian", // Make sure this matches your model name exactly
    })
    .sort({ createdAt: -1 })
    .limit(5);

  // Filter out bookings with invalid servian data
  const validRecentBookings = recentBookings.filter(
    (booking) => booking.servian && booking.servian._id && booking.servian.name
  );
  ...
  const pendingBookings = await Booking.find({
    ...
    .populate({
      path: "servian",
      select: "name profileImage phone role serviceCategory",
      model: "servian",
    })
    .sort({ scheduledDate: 1 });

  // Filter pending bookings as well
  const validPendingBookings = pendingBookings.filter(
    (booking) => booking.servian && booking.servian._id && booking.servian.name
  );
  ...
  const dashboardData = {
    ...
    recentBookings: validRecentBookings,
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  const { name, phone, preferredServices, preferences, emergencyContact } =
    req.body;
...
export const getBookings = asyncHandler(async (req, res) => {
  ...
  const bookings = await Booking.find(query)
    .populate(
      "servian",
      "name profileImage averageRating phone serviceCategory role"
    )
    .sort({ createdAt: -1 })
    .skip((page - 1) * limit)
    .limit(Number(limit));
...
export const cancelBooking = asyncHandler(async (req, res) => {
  ...
  // Make reason optional with a default value
  ...
  try {
    ...
    if (location) {
      ...
      if (
        !isNaN(lat) &&
        !isNaN(lng) &&
        typeof Servian.findNearbyServians === "function"
      ) {
...
export const getServianDetails = asyncHandler(async (req, res) => {
  ...
  const servian = await Servian.findById(id).select(
...
export const getFavorites = asyncHandler(async (req, res) => {
  const customer = await Customer.findById(req.user.id).populate(
    "favoriteServians.servian",
    "name profileImage averageRating serviceCategory pricing availability role"
  );
